  ...
  try {
    ...
    if (format === 'csv') {
      ...
      const formatMatch = (m: any) => {
        ...
        if (m.player3_id) text += ` vs ${m.player3_id}`;
        if (m.player4_id) text += ` vs ${m.player4_id}`;
        text += ` (${m.score1}:${m.score2}`;
        if (m.score3 !== null && m.score3 !== undefined) text += `:${m.score3}`;
        if (m.score4 !== null && m.score4 !== undefined) text += `:${m.score4}`;
        text += `)`;
        if (m.winner_id) text += ` Winner: ${m.winner_id}`;
        return text;
      };
...
export const recordMatch = async (req: Request, res: Response) => {
  try {
    ...
    let playerCount = 2;
    if (player3_id) playerCount = 3;
    if (player4_id) playerCount = 4;

    // Determine winner (highest score)
    const participants: { id: string; score: number }[] = [
...
export const getPositionWinRates = async (req: Request, res: Response) => {
  try {
    ...
    const result = await query(queryStr, [pc]);
...
const ANALYTICS_JSON_PATH = path.resolve(__dirname, '../../data/analytics-stats.json');
...
export const syncMergedStatsJson = async (req: Request, res: Response) => {
  try {
    ...
    await fs.mkdir(path.dirname(ANALYTICS_JSON_PATH), { recursive: true });
    await fs.writeFile(ANALYTICS_JSON_PATH, JSON.stringify(payload, null, 2), 'utf-8');
...
export const getMergedStats = async (req: Request, res: Response) => {
  try {
    const content = await fs.readFile(ANALYTICS_JSON_PATH, 'utf-8');
